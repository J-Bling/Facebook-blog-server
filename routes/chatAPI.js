...
class ChatBoxs {
    ...
    handleJoinRoom(chatId, socket, userId) {
        ...
        socket.to(chatId).emit("friendStatus", { message: `you friend had in the room` ,value:true});
    }
    ...
    async handleSendMessage(chatId, content, socket, userId, friendId) {
        ...
        socket.to(chatId).emit("friendStatus",{ message: '' ,value:true});
    ...
    startClearCache() {
        ...
        this.cacheTimeId = setInterval(async () => {
            try{
                ...
                console.error("insertMessage_error",error);
            }
        }, this.clearTime);
    }
        ...
        try{
            ...
            for(const [key,value] of this.chatUpdate.entries()){
                if(value.messages.length>0){
                    ...
                    let Obj=this.chatUpdate.get(key);
                    Obj.message=[];
                }
            }
            if(updates.length>0) await Chat.bulkWrite(updates);
        }catch(e){
            console.error("insertChat_error",e);
        }
    }
    ...
    updateChatBox() {
        try{
            ...
            console.error("upodateChatBox_error",e);
        }
    }
